...
def test_direct_api_cannot_invite_or_remove_in_another_league(app):
    ...
    assert api.handle(
        ...
        {"email": "member@example.com", "expires_at": expires},
    ).status == 404
    assert api.handle(
...
def test_invitation_is_bound_to_email_and_grants_membership(app):
    _, _, _, api, _, _, _, owner_token, outsider_token, member_token = app
    league_id = api.handle(
        "POST", "/api/v1/leagues", owner_token, {"name": "Privada"}
    ).body["id"]
    ...
    assert api.handle("GET", f"/api/v1/leagues/{league_id}", member_token).status == 200
...
def test_free_limits_are_transactional_and_count_pending_invites(app):
    ...
    leagues.invite(
        owner.id, league.id, "member@example.com", NOW + timedelta(days=1), NOW
    )
...
def test_member_cannot_invite_and_deleted_session_is_revoked(app):
    ...
    with pytest.raises((Forbidden, NotFound)):
        leagues.invite(
            member.id, league.id, "x@example.com", NOW + timedelta(days=1), NOW
        )
